...
class Cliente(models.Model):
    ...
    def api_key(self):
        if self.apikey_set.exists():
            ...
            a = APIKey(cliente=self, key=hashlib.sha1("%d-%s-%s"%(self.id, self.full_name(), datetime.today())).hexdigest())
            a.save()
            return a
...
class Operacion(models.Model):
    ...
    def __unicode__(self):
        return "%s - %10.2f - %s - %s"%(self.cliente,float(self.valor),self.descripcion,self.fecha.strftime("%d-%m-%Y"))
