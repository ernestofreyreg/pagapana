    ...
    if Operacion.objects.filter(weburl=weburl).exists():
        ...
        if request.method == 'POST':
            ...
            if usuario and usuario.id==data['cliente'].usuario.id:
                # Validada Clave
                pago = Pago(operacion=operacion, comprador=data['cliente'], pagado=operacion.valor-COMISION_TRANSACCION)
                pago.save()
                ...
                manager.save()
                deposito = Deposito(cliente=manager, valor=COMISION_TRANSACCION, motivo="COMISION PAGO #%d"%(pago.id))
                deposito.save()
                messages.info(request, "Realizado pago en Operacion: %s por %10.2f"%(operacion.descripcion, float(operacion.valor)))

                # Enviar correo
                # Al Pagador
                send_html_mail("Pago realizado PAGAPANA.TK", "emails/pagorealizado.html", {"pago": pago}, [pago.comprador.email()] )
                # Al Cobrador
                send_html_mail("Pago recibido PAGAPANA.TK", "emails/pagorecibido.html", {"pago": pago}, [pago.operacion.cliente.email()] )

                if 'redirect' in request.POST:
                    return HttpResponseRedirect("%s?weburl=%s?pagado"%(request.POST['redirect'],operacion.weburl))
                else:
            ...
            data['formulario_registro'] = RegistroFormulario(initial={"tipoidentificacion": TipoIdentificacion.objects.first()})

            return render_to_response("pago/pago.html", data, context_instance=RequestContext(request))
    else:
        return render_to_response("pago/noexiste.html", data, context_instance=RequestContext(request))
